				...
				, success: function (data) {
					...
					if (data.items.count > 0) {	// 해당 사업자번호로 조회되는 결과가 있을 경우
						...
						if (nice_operYn == "N") { // 폐업 상태일 경우 가입 불가 처리
							...
							var txtNotice = "휴업/폐업 사업자등록번호로 확인되어 가입이 불가능 합니다.\n기업정보 정정이 필요한 경우 나이스평가정보㈜로 문의 바랍니다.\n- 고객센터 전 화 : 02-3771-1514\n- 고객센터 이메일 : help@nicebizinfo.com";
					...
					}else {	// 나신평 기업정보 API에서 해당 사업자번호로 조회되는 결과가 없다면

						$.ajax({ // 국세청 기업정보 오픈 API를 톻한 2차 검증
							type: "POST"
							, url: "Ajax_DataGov_BizNum_Chk.asp"
							, data: { biznum: inputValue }
							, dataType: "text"
							, async: true
							, success: function (data) {
								...
								if (data.trim() == "S") {
									...
									// 국세청 기업정보 API 에서 조회되는 결과가 있다면 입력란 활성화 - 기업정보 수기입력 허용, 고객센터에서 기업정보 최종 확인 후 공고 등록할 수 있도록 회사등급 심사 상태로 설정
									$("#bizNumAuthChk").val("1");
									$("#bizNumAuthType").val("D");

									$tooltip.attr('data-params', 'success');
									var txtNotice2 = "사업자등록번호에 대한 확인이 완료되었습니다.\n회원 가입 후 공고 등록, 유료 서비스 결제 등 일부 서비스 이용 시\n추가 인증이 필요할 수 있습니다.";
									txtNotice2 = txtNotice2.replace(/(?:\r\n|\r|\n)/g, '<br />');
									$("#biznum_box").html(txtNotice2);
									$("#biznum_box").show();
									...
									// 기업정보 API 에서 조회되는 결과가 없다면
									$("#bizNumAuthChk").val("");
									$("#bizNumAuthType").val("");

									$tooltip.attr('data-params', 'error');
									var txtNotice3 = "해당 사업자등록번호는 나이스평가정보㈜에서 조회되지 않는 정보입니다.\n기업정보 추가가 필요한 경우 나이스평가정보㈜로 문의 바랍니다.\n- 고객센터 전 화 : 02-3771-1514\n- 고객센터 이메일 : help@nicebizinfo.com";
									txtNotice3 = txtNotice3.replace(/(?:\r\n|\r|\n)/g, '<br />');
									$("#biznum_box").html(txtNotice3);
									$("#biznum_box").show();
					...
					<div class="innerWrap">
						<div class="signUpBoxArea">
							<div class="signBoxInner">
								...
								<input type="hidden" id="bizNumAuthType" name="bizNumAuthType" value="" /><%'기업정보 API 인증 구분(N: 나신평 인증, D: 국세청 인증)%>
								<input type="hidden" id="bizNumAuthChk" name="bizNumAuthChk" value="" /><%'사업자번호 검증 여부(0/1)%>
								<input type="hidden" id="hidBizNum" name="hidBizNum" value="" /><%'사업자번호(최종 입력)%>
									...
									<div class="infMsg">
										<div class="cmmInput radiochk tp99">
											<label>
												...
												<span class="lb">이벤트 및 인사자료와 지원 관련 안내 메일 수신에 동의합니다. (선택)</span>
											</label>
										</div>
									</div>
